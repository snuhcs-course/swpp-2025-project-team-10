...
class HomeFragmentTest {
    ...
    fun swipeRefresh_triggersLoadFeed() {
        ...
        // Then
        runBlocking {
            org.mockito.Mockito.verify(mockRepo, org.mockito.Mockito.atLeast(2)).getFeed()
        }
    }
        ...
        onView(withId(R.id.rvFeed))
            .perform(
                RecyclerViewActions.actionOnItemAtPosition<RecyclerView.ViewHolder>(
                    0,
                    com.example.librarytogether.util.CustomViewActions.clickChildViewWithId(R.id.btnAdd)
                )
            )
