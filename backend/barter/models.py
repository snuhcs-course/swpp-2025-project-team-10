...
class BarterCounter(models.Model):
    ...
    preferred_meeting_type = models.CharField(
        max_length=20,
        choices=BarterRequest.MEETING_TYPE_CHOICES,
        blank=True
    )
    proposed_meeting_location = models.CharField(max_length=200, blank=True)
    proposed_meeting_time = models.DateTimeField(null=True, blank=True)
...
class BarterTransaction(models.Model):
    ...
    def is_completed(self):
        ...
        return (self.completion_confirmed_by_requester and
                self.completion_confirmed_by_recipient)
...
class BarterRating(models.Model):
    ...
    # Feedback
    feedback = models.TextField(blank=True, help_text="Optional feedback about the barter experience")
    ...
    def __str__(self):
        return f"Rating by {self.rater.username} for {self.rated_user.username}"
