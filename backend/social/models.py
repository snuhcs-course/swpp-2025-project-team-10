...
class Post(models.Model):
    ...
    def __str__(self):
        return f"Post by {self.author.username} - {self.get_post_type_display()}"
...
class BookClub(models.Model):
    ...
    # Club Settings
    is_public = models.BooleanField(default=True, help_text="Can anyone join this club?")
    max_members = models.IntegerField(null=True, blank=True, help_text="Maximum number of members (optional)")
...
class BookClubDiscussionReply(models.Model):
    ...
    author = models.ForeignKey(User, on_delete=models.CASCADE, related_name='club_replies')
    content = models.TextField()
...
class Recommendation(models.Model):
    ...
    # Recipient Response
    is_read = models.BooleanField(default=False)
    is_accepted = models.BooleanField(null=True, blank=True)  # True=accepted, False=declined, None=no response
