...
class HomeFeedEndpointTestCase(TestCase):
    ...
    def test_post_like_toggle(self):
        ...
        self.assertTrue(PostLike.objects.filter(post=self.post2, user=self.user).exists())
        ...
        self.assertFalse(PostLike.objects.filter(post=self.post2, user=self.user).exists())
