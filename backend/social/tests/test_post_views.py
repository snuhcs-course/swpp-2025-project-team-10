...
def test_update_post_with_invalid_data():
    ...
    )  # Might not have update endpoint
...
def test_home_feed_returns_multiple_posts():
    ...
    Post.objects.create(
        author=user, content="Post 1", is_public=True, post_type="text"
    )
    Post.objects.create(
        author=user, content="Post 2", is_public=True, post_type="text"
    )
    Post.objects.create(
        author=user, content="Post 3", is_public=True, post_type="book_review"
    )
