...
def test_with_django_client():
    ...
    if response.status_code == 200:
        ...
        print(f"   Error: {response.json() if response.status_code != 500 else response.content.decode()}")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "server":
            ...
            print("   python test_server.py test       - Test with Django client")
            print("   python test_server.py test-live  - Start server and test")
