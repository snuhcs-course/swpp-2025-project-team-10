...
def test_simple_auth():
    ...
    auth_result = authenticate(
        username="simple@test.com", password="simplepass123"
    )
    print(f"Direct authenticate result (with email): {auth_result}")

    # Also test with username (should fail)
    auth_result_username = authenticate(
        username="simpletest", password="simplepass123"
    )
    print(
        f"Direct authenticate result (with username): {auth_result_username}"
    )
    ...
    if auth_result is None:
        ...
        try:
            ...
            print("Backend cannot get user:", e)

        # Test backend authenticate method
        backend_auth = backend.authenticate(
            None, username="simpletest", password="simplepass123"
        )
        ...
        serializer_email = CustomTokenObtainPairSerializer(
            data={
        ...
        if serializer_email.is_valid():
            ...
            print(
                f"Custom serializer errors with email: {serializer_email.errors}"
            )

        print("\nTesting custom serializer with username:")
        serializer_username = CustomTokenObtainPairSerializer(
            data={
                "username": "simpletest",
                "password": "simplepass123",
            }  # Use username
        ...
        if serializer_username.is_valid():
            ...
            print(
                f"Custom serializer errors with username: {serializer_username.errors}"
            )
