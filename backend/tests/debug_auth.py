...
def debug_authentication():
    ...
    try:
        ...
        auth_user = authenticate(username='testuser123', password='testpass123')
        print(f"Django authenticate result: {auth_user}")

        # Test with email
        auth_user_email = authenticate(username=user.username, password='testpass123')
        print(f"Django authenticate with email lookup: {auth_user_email}")

        # Test authentication backends directly
        from django.contrib.auth.backends import ModelBackend
        backend = ModelBackend()
        auth_result = backend.authenticate(None, username='testuser123', password='testpass123')
        print(f"ModelBackend authenticate result: {auth_result}")

        # Check user model
        print(f"User model: {User}")
        print(f"User._meta.get_field('username'): {User._meta.get_field('username')}")
        ...
        fresh_auth = authenticate(username='freshuser', password='freshpass123')
        ...
        print(f"Password starts with hash: {user.password.startswith('pbkdf2_')}")
