...
def test_user_barter_info_includes_full_taste_and_counts():
    ...
    publication_a = BookPublication.objects.create(
        title="Book A",
        publisher=publisher,
    )
    publication_a.authors.add(author)
    b1 = BookCopy.objects.create(publication=publication_a, owner=other)

    publication_b = BookPublication.objects.create(
        title="Book B",
        publisher=publisher,
    )
    publication_b.authors.add(author)
